...
class torpedoforce:
    ...
    def waterforce(self, alpha, beta, wx, wy, wz, V):
        return self._steadyforce(alpha, beta, V)+self._unsteadyforce(wx, wy, wz, V)

    def _steadyforce(self, alpha, beta, V):
        alphapoint, betapoint = self._alpha_beta_striction(alpha, beta)
        ...
        Mx_stea = 1/2*self.rho*V**2*self.S_ref * \
            self.L_ref*self.Cmx(alphapoint, betapoint)
        My_stea = 1/2*self.rho*V**2*self.S_ref * \
            self.L_ref*self.Cmy(alphapoint, betapoint)
        Mz_stea = 1/2*self.rho*V**2*self.S_ref * \
            self.L_ref*self.Cmz(alphapoint, betapoint)
        return np.array([X_stea, Y_stea, Z_stea, Mx_stea, My_stea, Mz_stea])

    def _unsteadyforce(self, wx, wy, wz, V):
    ...
    def _alpha_beta_striction(self, alpha, beta):
        # alpha(rad) beta(rad)
        if (alpha*57.3) >= 18:
            alpha = 18/57.3
        if (alpha*57.3) <= -18:
    ...
    def tailrudderforce(self, u, V):
        ...
        dX_rudder = 1/2*1/2*self.rho*V**2 * \
            self.S_ref*self.dcx_dz*(abs(dzl)+abs(dzr))
        dY_rudder = 1/2*1/2*self.rho*V**2*self.S_ref*self.dcy_dz*(dzl+dzr)
        dMz_rudder = 1/2*1/2*self.rho*V**2 * \
            self.S_ref*self.L_ref*self.dmz_dz*(dzl+dzr)
